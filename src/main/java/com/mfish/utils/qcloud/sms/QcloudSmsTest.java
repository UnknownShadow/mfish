package com.mfish.utils.qcloud.sms;
...
/**
 * 腾讯云短信SDK
 * 注意：发送消息的内容跟创建的模板内容一致
 */
public class QcloudSmsTest {

    public static void main(String[] args) {
        // 短信应用SDK AppID
        int appid = 1400092712; // 1400开头

        // 短信应用SDK AppKey
        String appkey = "0c6128a191850ace9f9c6f9685d0c9d2";

        // 需要发送短信的手机号码
        String[] phoneNumbers = {"15073199915", "13142274876"};

        // 短信模板ID，需要在短信应用中申请
        // NOTE: 这里的模板ID`7839`只是一个示例，
        // 真实的模板ID需要在短信控制台中申请
        int templateId = 123594;

        // 签名
        // NOTE: 这里的签名"腾讯云"只是一个示例，
        // 真实的签名需要在短信控制台中申请，另外
        // 签名参数使用的是`签名内容`，而不是`签名ID`
        String smsSign = "盛悦化工科技有限公司";

        // 单发短信
        try {
            SmsSingleSender ssender = new SmsSingleSender(appid, appkey);
            SmsSingleSenderResult result = ssender.send(0, "86", phoneNumbers[0],
                "“你的验证码为：999999，请于1分钟内填写。如非本人操作，请忽略本短信。", "", "");
